...
public class CogWheelFlag extends BlockFlag implements IPartialHolder {
    ...
    private JsonElement getModel(Material material) {
        boolean texturePresent = Minecraft.getInstance().getResourceManager().getResource(new ResourceLocation(material.getNamespace() + ":textures/block/materials/" + material.getName() + "/cogwheel.png")).isPresent();
        String texture = texturePresent ? material.getNamespace() + ":block/materials/" + material.getName() + "/cogwheel" : "rutile:block/materials/null/cogwheel";
        JsonObject model = new JsonObject();
        model.addProperty("parent", "metallurgica:block/template/cogwheel/"+getCogWheelModelVariant()+"/small");
    ...
    public FlagKey<?> getKey() {
        return MetallurgicaFlagKeys.COG_WHEEL;
    }
